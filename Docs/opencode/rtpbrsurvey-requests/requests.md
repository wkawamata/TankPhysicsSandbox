# Requests To RtPbrSurvey (OpenCode Mirror)

Source: `Docs/rtpbrsurvey-requests/requests.md`
...
[2026-07-15] Verified the submodule at `f90591d` (latest on `origin/main`) already contains `SceneRenderer`, `SceneBuilder`, `SetToolUiHandler`, and `DebugCameraController`. The API surface mentioned in this request is present. The remaining gap is build-system integration (CMake target or source-list include), not API availability.

[2026-07-15] `SceneRenderer` wraps `RtPbrSurveyEngine`. Host owns `GraphicsDevice`, passes it to `SceneRenderer` constructor. `RunFrame()` takes a `UiRenderHandler` lambda for ImGui rendering. `SetToolUiHandler()` provides a separate callback for tool UI drawn inside the engine's ImGui pass.
