...
`Vehicle Telemetry > Rolling` に開始診断と Rolling Travel を集約しました。
`Input / Start` は左右レバーX、入力再受付、保持要求、ブレーキ、Phase、要求拒否理由、
`Stop gate` は速度・角速度・履帯滑り・サスペンション速度・停止確認進捗・停止条件の失敗理由、
`Motion / Travel` は移動距離・目標距離・開始姿勢からの角度・Roll角速度を表示します。
各グラフは Speed と共通の Time Window を使い、ラベルは内側に表示します。マウスを重ねると値と計測時刻を確認できます。

走行中の同方向レバー入力も一度だけ受け付け、駆動入力を抑えてフルブレーキを掛けます。
レバーを戻しても要求方向を保持し、既存の停止条件（接地、姿勢、速度、滑り、サスペンション、確認時間）が成立すると Rolling を開始します。
速度を直接ゼロにはせず、完全な数値ゼロではなく設定済み停止閾値で判定します。
安全条件が成立しない間は待機を継続し、Reset Tank で保持要求を破棄します。実行中の追加要求は予約しません。
左右レバーをともに ±0.20 以内へ戻すと再受付可能になり、同方向へ ±0.70 以上倒すと要求します。
